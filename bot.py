import os, hmac, hashlib, asyncpg, asyncio, threading
...
def _get_bot_app():
    ...
    if _bot_app is None:
        ...
        _bot_app.add_handler(CommandHandler("balance", balance_command))
        _bot_app.add_handler(CommandHandler("tasks", tasks_command))
        _bot_app.add_handler(CommandHandler("referral", referral_command))
        _bot_app.add_handler(CommandHandler("withdraw", withdraw_command))
        _bot_app.add_handler(CommandHandler("top", top_command))
        _bot_app.add_handler(CommandHandler("support", support_command))
...
def callback():
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            _handle_callback(uid, amount, tx, status),
            _loop
        )
